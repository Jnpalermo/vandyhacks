...
class ImageCarousel extends React.Component {
    ...
    if (prevProps.imgResponse.length != 0) {
      ...
      if (word_split.length == 2) {
        ...
        if (word_split[0] == prevState.selected.name) {
          //Move to next slide
          this.setState({
            activeIndexSlider: this.state.activeIndexSlider + 1,
          });
        } else {
          //Keep on trying
        }
  ...
  render() {
    ...
    const imageListItems = this.props.imageList.map((image) => (
      <Carousel.Item>
        <img
          ...
          alt={image.name}
        />
        <Carousel.Caption>{/* <h3>{image.name}</h3> */}</Carousel.Caption>
      </Carousel.Item>
    ));
    return (
      <Carousel
        activeIndex={this.state.activeIndexSlider}
